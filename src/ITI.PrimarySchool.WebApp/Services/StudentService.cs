...
namespace ITI.PrimarySchool.WebApp.Services
{
    public class StudentService
    {
        ...
        public StudentService( StudentGateway studentGateway, ClassGateway classGateway )
        {
        ...
        public Result AssignStudentClass(int studentId, int classId)
        {
            Class c = null;
            if (_studentGateway.FindById(studentId) == null) return Result.Failure(Status.BadRequest, "Unknown student.");
            if (classId != 0 && (c = _classGateway.FindById(classId)) == null) return Result.Failure(Status.BadRequest, "Unknown class.");
            _studentGateway.AssignStudentClass(studentId, classId);
            return Result.Success(Status.Ok);
        }

        public Result<Class> AssignedStudentClass(int studentId)
        {
            if (_studentGateway.FindById(studentId) == null) return Result.Failure<Class>(Status.BadRequest, "Unknown student.");
            Class c = _classGateway.FindById(_studentGateway.FindById(studentId).ClassId);
